...
function logConsole(msg, payload, severity, scope, execution_id){  
    var env_userName = typeof process.env['USERPROFILE'] !== 'undefined' && process.env['USERPROFILE'] ? process.env['USERPROFILE'] : process.env['HOME'];
    var userName = typeof env_userName.split(path.sep)[2] !== 'undefined' && env_userName.split(path.sep)[2] ? env_userName.split(path.sep)[2] : env_userName.split(path.sep)[1];

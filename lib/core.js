...
const { ServerResponse, ClientRequest, IncomingMessage } = require('http'); 
...
function before_middleware(request, response, next){
    ...
    try { trace_db(request); } catch(err){ console.log(err) }
    
    next();
}

function finally_middleware(request, response, next){    
    try { trace_db(response); } catch(err){ console.log(err) }
...
function trace_db(http_package){
    if(!http_package)
        return;

    if(http_package instanceof ClientRequest || http_package instanceof IncomingMessage)
    {
    ...
    else if(http_package instanceof ServerResponse)
    {           
        ...
        sequelize.query('INSERT INTO [Response] ([OperationId], [Timestamp], [Status], [RemoteIp], [RemotePort], [LocalIp], [LocalPort], [Headers], [Body]) VALUES (:operationid, :timestamp, :status, :remoteip, :remoteport, :localip, :localport, :headers, :body)',
        {
            replacements: { 
                ...
                body: http_package.hasBody ? JSON.stringify(http_package.body) : null
            }, 
            type: sequelize.QueryTypes.INSERT 
        }).catch(function(err){ /*console.log(err);*/ });
    }
